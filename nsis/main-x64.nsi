...
!define MUI_FINISHPAGE_SHOWREADME "$PROGRAMFILES64\${APPLI}\README.TXT"
...
Section "ActivePerl"
	...
	ExecWait 'msiexec /quiet /l* "$TEMP\perl.log" /norestart /i "$TEMP\${ACTIVEPERL}" PERL_PATH=Yes PERL_EXT=Yes TARGETDIR="$PROGRAMFILES64" ADDLOCAL="PERL_FEATURE"'
...
Section "WinAdminPassword"
	...
	SetOutPath "$PROGRAMFILES64\${APPLI}"
	SetOverwrite on
	File "..\${MAKEADMINPASSWD}"
	File "..\*.TXT"
	Delete "$PROGRAMFILES64\${APPLI}\${MAKEADMINPASSWD}.pl"
	Rename "$PROGRAMFILES64\${APPLI}\${MAKEADMINPASSWD}" "$PROGRAMFILES64\${APPLI}\${MAKEADMINPASSWD}.pl"

	; $PATH
	${EnvVarUpdate} $0 "PATH" "A" "HKLM" "$PROGRAMFILES64\${APPLI}"
	${EnvVarUpdate} $0 "PATHEXT" "A" "HKLM" ".PL"

	; Création du désinstalleur
        WriteUninstaller "$PROGRAMFILES64\${APPLI}\uninstall.exe"
        ...
        WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}" "DisplayIcon" '"$PROGRAMFILES64\${APPLI}\uninstall.exe"'
        ...
        WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}" "UninstallString" '"$PROGRAMFILES64\${APPLI}\uninstall.exe"'
SectionEnd

; Section de désinstallation
Section "Uninstall"
        RMDir /r "$PROGRAMFILES64\${APPLI}"
        Delete "$PROGRAMFILES64\${APPLI}\uninstall.exe"
        DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}"
	${un.EnvVarUpdate} $0 "PATH" "R" "HKLM" "$PROGRAMFILES64\${APPLI}"
...
Function .onInit
	; If 64 Bits
	IfFileExists "$WINDIR\SysWOW64\svchost.exe" +3 +1
		MessageBox MB_OK "You must have a 64 bits system to execute this file"
		Abort
