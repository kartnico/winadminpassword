...
!include "variables-without-perl.nsh"
...
!define MUI_FINISHPAGE_SHOWREADME "$InstallPath\${APPLI}\README.TXT"
...
Section -Main
	...
	IfFileExists "$WINDIR\SysWOW64\svchost.exe" x64 x86
        x64:
                StrCpy $InstallPath "$PROGRAMFILES64"
                Goto Next
        x86:
                StrCpy $InstallPath "$PROGRAMFILES"
                Goto Next
        Next:
SectionEnd
...
Section "WinAdminPassword"
	...
	SetOutPath "$InstallPath\${APPLI}"
	SetOverwrite on
	File "..\${MAKEADMINPASSWD}"
	File "..\*.TXT"
	Delete "$InstallPath\${APPLI}\${MAKEADMINPASSWD}.pl"
	Rename "$InstallPath\${APPLI}\${MAKEADMINPASSWD}" "$InstallPath\${APPLI}\${MAKEADMINPASSWD}.pl"

	; $PATH
        ${EnvVarUpdate} $0 "PATH" "A" "HKLM" "$InstallPath\${APPLI}"
        ${EnvVarUpdate} $0 "PATHEXT" "A" "HKLM" ".PL"

	; Création du désinstalleur
        WriteUninstaller "$InstallPath\${APPLI}\uninstall.exe"
        ...
        WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}" "DisplayIcon" '"$InstallPath\${APPLI}\uninstall.exe"'
        ...
        WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}" "UninstallString" '"$InstallPath\${APPLI}\uninstall.exe"'
SectionEnd
...
Section "Uninstall"
	...
	IfFileExists "$WINDIR\SysWOW64\svchost.exe" x64 x86
        ...
        x86:
                ...
                Goto End
        End:
	        RMDir /r "$InstallPath\${APPLI}"
		Delete "$InstallPath\${APPLI}\${MAKEADMINPASSWD}.pl"
	        Delete "$InstallPath\${APPLI}\uninstall.exe"
	        DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_APPLI}"
		${un.EnvVarUpdate} $0 "PATH" "R" "HKLM" "$InstallPath\${APPLI}"
